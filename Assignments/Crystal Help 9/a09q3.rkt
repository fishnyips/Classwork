...
;; (students-in-course coursesub coursenum los) produces a (listof Student)
;; who are registered in a coursesub as well as the corresponding coursenum
;; from a los
;; students-in-course: Symbol Nat (listof Students) -> (listof Students)

;; Examples:
(check-expect (students-in-course 'CS 115 students) (list stu1 stu2))
(check-expect (students-in-course 'MATH 492 students) empty)

(define (students-in-course coursesub coursenum los)
  (filter (lambda (student)
            (not (equal? (filter
                          (lambda (course) (and (equal? coursesub (course-subject course))
                                                (equal? coursenum (course-number course))))
                          (student-courses student)) empty))) los))

;; Tests:
(check-expect (students-in-course 'MATH 659 students) empty)
(check-expect (students-in-course 'MATH 135 students) (list stu1 stu3 stu4))
(check-expect (students-in-course 'MATH 135 students) (list stu1 stu3 stu4))
(check-expect (students-in-course 'CS 135 students) (list stu4))
